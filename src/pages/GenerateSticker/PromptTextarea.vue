...
const generateSticker = async () => {
  ...
  try {
    ...
    stickerUrl.value = response.data.output[0]; // assuming output is an array
  ...
  >
    ...
    >
      ...
      <div class="flex justify-end px-4 py-3 w-full">
        <button
          :disabled="isLoading"
          class="flex gap-2 h-8 shrink-0 select-none items-center justify-center rounded-lg border border-zinc-950/10 bg-transparent px-2 text-sm text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-800 focus-visible:ring-2 active:scale-[0.98] dark:border-zinc-50/10 dark:text-zinc-50"
          type="submit"
        >
