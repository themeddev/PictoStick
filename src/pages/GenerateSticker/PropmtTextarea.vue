...
const stickerUrl = ref(null); // Store the generated sticker URL
const isLoading = ref(false); // Handle loading state
...
// Function to submit the prompt and generate sticker
const generateSticker = async () => {
  if (!prompt.value) return; // Don't proceed if prompt is empty

  isLoading.value = true; // Set loading to true while the API is working
  
  const replicate = new Replicate({
    auth: import.meta.env.VUE_APP_REPLICATE_API_TOKEN // Use the environment variable
  });

  try {
    const input = {
      prompt: prompt.value, // Pass the prompt to the API
      output_quality: 100
    };
    ...
    // Assuming output contains the sticker URL
    stickerUrl.value = output[0]; // Save the generated sticker URL
    console.log(output);
  } catch (error) {
